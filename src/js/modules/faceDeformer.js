...
var faceDeformer = function() {
  ...
  this.load = function(element, points, pModel, vertices) {
    ...
    if (first) {
      ...
      var fragmentShaderProg = [
        ...
        "  gl_FragColor.a = (v_alpha - 1.0) * (v_alpha - 1.0) * (v_alpha - 1.0) * (v_alpha - 1.0) * (v_alpha - 1.0) + 1.0;",
        "}"
      ].join('\n');
  ...
  this.draw = function(points) {
    ...
    var alphaLocation = gl.getAttribLocation(drawProgram, "a_alpha");
    ...
    // Create a buffer for the alpha of the vertices.
    var alphas = [];
    for (var i = 0;i < verticeMap.length;i++) {
      for (var j = 0;j < 3;j++) {
        alphas.push((verticeMap[i][j] > 22) * 1);
      }
    }
    ...
    console.log(numTriangles*4, vertices.length);
